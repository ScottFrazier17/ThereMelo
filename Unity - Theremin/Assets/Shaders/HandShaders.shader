Shader "Custom/HandShaders"
{
	Properties
	{
		...
		_StippleScale("Stipple Scale", Float) = 10
		_StippleThreshold("Stipple Threshold", Range(0,1)) = 0.5
	}
...
#include "AutoLight.cginc" // for UNITY_SHADOW_COORDS() & UNITY_TRANSFER_SHADOW()
			...
			SubShader
		{
			...
			Pass
			{
				...
				{
					float linePattern = frac(sin(dot(i.uv, float2(12.9898, 78.233))) * 43758.5453);
					linePattern = step(_StippleThreshold, linePattern) * _StippleScale;
					clip(linePattern - 0.5);
			 ...
			 Pass
			{
				...
				half4 frag(v2f i) :COLOR
				{
					// Zebra Pattern Effect
					float stripeWidth = 0.01; // Adjust stripe width
					float stripes = sin(i.uv.x * 2 * 3.14159 / stripeWidth);
					clip(stripes - 0.5); // Clip half of the stripes, adjust for different effects
